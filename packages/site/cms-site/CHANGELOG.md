...
-   4f32ad01: Bump styled-components peer dependency to v6
...
-   9351b145: Upgrade to Next 14 and React 18

    Add "use client" directive to components that currently require it (as they use styled-components or a context)

-   86358bf: Fix an issue where the block preview could break a block's styling and HTML structure
...
-   6986cdc: Require `aspectRatio` prop for `PixelImageBlock` and `Image`
...
-   36cdd70: Deprecate `InternalLinkBlock` component, instead there should be a copy of this component in the application for flexibility (e.g., support for internationalized routing)
-   6d56606a: Store site preview scope in cookie and add `previewParams()` helper to access it
...
-   a80ab10f: Add GraphQL fetch client
...
-   fc27014b: Add new technique for blocks to load additional data at page level when using SSR
