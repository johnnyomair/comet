...
-   9a1b0166: Remove CDN config from DAM
...
-   ebf59712: Make `@nestjs/platform-express` a peer dependency

    Make sure that `@nestjs/platform-express` is installed in the application.

-   e159275: Support "real" dependency injection in `BlockData#transformToPlain`
...
-   9bed7563: API Generator: Add new `dedicatedResolverArg` option to `@CrudField` to generate better API for Many-to-one-relations

    -   Add foreign id as argument to create mutation
    -   Add foreign id as argument to list query

-   28322b42: Refactor auth-decorators
...
-   2497a062: API Generator: Remove support for `visible` boolean, use `status` enum instead.
...
-   f1d0f02: API Generator: Add `list` option to `@CrudGenerator()` to allow disabling the list query

    Related DTO classes will still be generated as they might be useful for application code.

-   36cdd70: InternalLinkBlock: add scope to targetPage in output

    This allows for example using the language from scope as url prefix in a multi-language site

-   1d2f54b: Require `strategyName` in `createStaticCredentialsBasicStrategy`
...
-   Updated dependencies [e159275]
-   Updated dependencies [ebf59712]
